...
mjit_compile_method_inner (MonoMethod *method, MonoDomain *target_domain, gint32 opt, MonoError *error)
{
	...
	if (*(gint16*)mono_object_unbox_internal (ret) != 0 /* CompilationResult.Ok */) {
		/* set error */
		return NULL;
	}
...
ves_icall_mjit_execute_installed_method (InstalledRuntimeCode *irc, MonoArray *args)
{
	...
	MonoMethodSignature *sig = mono_method_signature_internal (jinfo->mjit_method);
	...
	if (args) {
		params = (void **) alloca (sizeof (gpointer) * mono_array_length_internal (args));
		gboolean has_byref_nullables = FALSE;

		for (int i = 0; i < mono_array_length_internal (args); i++) {
			MonoType *t = sig->params [i];
...
mini_runtime_register_icalls (void)
{
	...
	mono_add_internal_call_internal ("Mono.Compiler.RuntimeInformation::mono_install_compilation_result", ves_icall_mjit_install_compilation_result);
	mono_add_internal_call_internal ("Mono.Compiler.RuntimeInformation::mono_execute_installed_method", ves_icall_mjit_execute_installed_method);
