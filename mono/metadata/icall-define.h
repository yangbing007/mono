...
#define MONO_ICALL_DEFINE(ReturnSpec, name, ParametersSpec, Body)	\
	...
	{								\
		...
		Body ;							\
		MONO_ICALL_IMPL_RELEASE_PARAMETERS(ParametersSpec);	\
		MONO_ICALL_IMPL_ELEVATE_RETURN(ReturnSpec);		\
...
#define MONO_ICALL_IMPL_RELEASE_PARAMETERS(ParametersSpec) MONO_ICALL_IMPL_RELEASE_PARAMETERS_ ParametersSpec
#define MONO_ICALL_IMPL_RELEASE_PARAMETERS_(...)			\
	MONO_PP_VA_FOREACH(MONO_ICALL_IMPL_RELEASE_PARAMETER, __VA_ARGS__)

#define MONO_ICALL_IMPL_RELEASE_PARAMETER(Spec)			\
	MONO_PP_IF(MONO_ICALL_IMPL_IS_VAL(Spec))(		\
		MONO_PP_VA_EMPTY(),				\
		MONO_PP_IF(MONO_ICALL_IMPL_IS_REF(Spec))(	\
			MONO_PP_VA_APPLY_PACK(MONO_ICALL_IMPL_RELEASE_PARAMETER_REF, (MONO_ICALL_IMPL_ARGS_OF_REF(Spec))), \
			__error_in_icall_release_parameter__wanted_REF_or_VAL_spec))


#define MONO_ICALL_IMPL_RELEASE_PARAMETER_REF(Typ, name) \
	MONO_ICALL_IMPL_MANGLED_REF_NAME(name) = (Typ*) MONO_LH_RELEASE_TO_UNSAFE((MonoHandle*) name);


/* TODO: implement REF return specs */
#define MONO_ICALL_IMPL_DECLARE_RETURN_LCL(Spec)			\
	MONO_PP_IF(MONO_ICALL_IMPL_IS_REF(Spec))(			\
		__error_icall_with_REF_return_spec_unimplemented,	\
		MONO_PP_IF(MONO_ICALL_IMPL_IS_VAL(Spec))(		\
...
#define MONO_ICALL_IMPL_ELEVATE_RETURN(Spec)				\
	MONO_PP_IF(MONO_ICALL_IMPL_IS_REF(Spec))(			\
		__error_icall_with_REF_return_spec_unimplemented,	\
		MONO_PP_IF(MONO_ICALL_IMPL_IS_VAL(Spec))(		\
			MONO_PP_VA_APPLY_PACK(MONO_ICALL_IMPL_ELEVATE_RETURN_VAL, (MONO_ICALL_IMPL_ARGS_OF_VAL(Spec))), \
			__error_in_icall_elevate_return__wanted_REF_or_VAL_spec))

#define MONO_ICALL_IMPL_ELEVATE_RETURN_VAL(Typ, name) /*empty*/
 ...
 *   LocalHandle h1 = { 0 };
 *   LocalHandle h2 = { 0 };
 *   LocalHandle * __pushed_localhandles[] = { &h1, &h2, };
 *   const unsigned int __pushed_localhandles_count = sizeof(__pushed_localhandles[]) / sizeof(__pushed_localhandles[0]);
 *   MONO_HANDLE_ARENA_START(__pushed_localhandles_count);
 */
#define MONO_ICALL_PUSH(...)			\
{						\
	MONO_PP_VA_FOREACH(MONO_ICALL___DEFINE_LOCALHANDLE_STMT, __VA_ARGS__)	\
	LocalHandle *__pushed_localhandles[] = { MONO_PP_VA_FOREACH(MONO_ICALL___ADDR_OF_HANDLE_COMMA, __VA_ARGS__) }; 	\
	const unsigned int __pushed_localhandles_count = sizeof(__pushed_localhandles) / sizeof(__pushed_localhandles[0]) ; \
	MONO_HANDLE_ARENA_START(__pushed_localhandles_count);

#define MONO_ICALL___DEFINE_LOCALHANDLE_STMT(h) LocalHandle h = { 0 };

#define MONO_ICALL___ADDR_OF_HANDLE_COMMA(h) &h,

/*
 * MONO_HANDLE_ARENA_END
 */
#define MONO_ICALL_POP()						\
	MONO_HANDLE_ARENA_END;						\
}
...
{
	MonoObject *obj = mono_handle_obj(h);
	h->obj = NULL;
	return obj;
}
...
#  define MONO_LH_GET(h,ty,fieldname) ((ty*)mono_handle_obj (h))->fieldname

#  define MONO_LH_SETV(h,ty,fieldname,val) ((ty*)mono_handle_obj (h))->fieldname = val

#  define MONO_LH_SETREF(h,ty,fieldname,h2)				\
	do {								\
		MONO_PREPARE_RESET_BLOCKING				\
		ty *__lh_sr_ptr = (ty *)mono_gchandle_get_target(h.gchandle); \
		MONO_OBJECT_SETREF(__lh_sr_ptr, fieldname, mono_gchandle_get_target(h2.gchandle)); \
		MONO_FINISH_RESET_BLOCKING				\
	} while (0)

#  define MONO_LH_RELEASE_TO_UNSAFE(h) mono_lh_release (h)

#  define MONO_LH_GET_TARGET(h) mono_handle_obj(h)
